...
export default {
  ...
  methods: {
    drawBarChart() {
      ...
      const xScale = d3.scaleIdentity().domain([0, INNER_WIDTH]);
      ...
      (function update(data) {
        ...
        const bar = chartElement.selectAll("g").data(data, d => d.id);
        ...
        bar
          ...
          .attr(
            "transform",
            (d, i) => `translate(${i * (BAR_WIDTH + BAR_GAP) + 45},${y(d)})`
          );
        ...
        const barEnter = bar
          ...
          .attr(
            "transform",
            (d, i) =>
              `translate(${i * (BAR_WIDTH + BAR_GAP) + 45},${INNER_HEIGHT})`
          );
